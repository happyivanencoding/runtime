...
func (s *Server) RuntimeContext(ctx context.Context) (app.Result, error) {
	return s.runtime.RuntimeContext(ctx)
}

// AgentDockLocalContext 为 Nexus Bridge 私有操作提供节点本地 Context，
// 返回结构沿用 runtime_context 的标准工具结果 envelope。
func (s *Server) AgentDockLocalContext(ctx context.Context) (map[string]any, error) {
	...
	return toolEnvelope("runtime_context", result, err), nil
}
