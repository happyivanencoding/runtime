...
func OutputSchema(name string) map[string]any {
	...
	case "acp_start", "acp_resume", "acp_status", "acp_stop":
		props["action"] = stringProp("Completed Runtime ACP adapter action.")
		props["default_active"] = boolProp("Always false for the Runtime ACP adapter layer; agents are started only by explicit start/resume calls.")
		props["adapter"] = objectProp("Resolved ACP adapter identity and executable source without environment values.")
		props["adapters"] = arrayProp("Built-in ACP adapter availability without launching any agent.")
		props["agent"] = objectProp("ACP agent identity reported by the adapter.")
		props["session"] = objectProp("Persisted Runtime ACP session record.")
		props["sessions"] = arrayProp("Persisted Runtime ACP sessions across adapters.")
		props["count"] = intProp("Returned persisted session count.")
		props["run_id"] = stringProp("Current-process asynchronous ACP prompt run id.")
		props["status"] = stringProp("ACP session or prompt run status.")
		props["events"] = arrayProp("Ordered prompt events for a current-process run.")
		props["next_seq"] = intProp("Cursor for the next acp_status call.")
		props["has_more"] = boolProp("Whether more retained events are immediately available.")
		props["truncated"] = boolProp("Whether requested prompt history predates the retained event ring.")
		props["started_at"] = stringProp("Prompt run start timestamp.")
		props["ended_at"] = stringProp("Prompt run end timestamp when settled.")
		props["stop_reason"] = stringProp("ACP stop reason when present.")
		props["error_code"] = stringProp("ACP run error code when present.")
		props["message"] = stringProp("ACP run error message when present.")
		props["interactions"] = arrayProp("Pending permission interactions for the active in-process manager.")
		props["interaction_count"] = intProp("Pending permission interaction count.")
		props["modes"] = objectProp("Session modes reported by the ACP agent when present.")
		props["config_options"] = arrayProp("Session configuration options reported by the ACP agent when present.")
		props["stopped"] = boolProp("Whether acp_stop closed the session.")
	case "acp_session":
	...
	case "browser_session":
		...
		props["transport"] = stringProp("Browser transport, including extension for Runtime Chrome Bridge status.")
		props["connected"] = boolProp("Whether Runtime Chrome Bridge is connected.")
	...
	case "browser_act", "browser_step", "browser_snapshot":
		...
		props["dom"] = stringProp("Bounded serialized live DOM for the current document.")
		...
		props["network_events"] = arrayProp("Bounded successful/received network responses captured during this operation.")
		...
		props["downloads"] = arrayProp("Downloads initiated and completed by this browser operation.")
		props["download_artifacts"] = arrayProp("Published immutable Artifact references for completed downloads.")
		props["closed"] = boolProp("Whether close_after terminated the session after successful Artifact publication.")
		props["fallback_used"] = boolProp("Whether explicit Windows UI Automation fallback completed after a browser transport failure.")
