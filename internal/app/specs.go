...
func requiresJev(cfg config.Config) bool        { return cfg.TypeSafeAPIKey != "" }
func requiresJevBrowser(cfg config.Config) bool { return cfg.BrowserEnabled && requiresJev(cfg) }
...
func allToolSpecs() []ToolSpec {
	return bindToolSchemas(append([]ToolSpec{
		...
		{Name: mcpcontract.ToolAgentDockContext, Title: "Runtime context (compatibility)", Description: "Compatibility alias for runtime_context used by previously registered Runtime MCP clients.", Annotations: runtimeContextAnnotations(), Handler: ctxToolHandler((*Runtime).runtimeContextTool)},
		...
		{Name: "request_receipt", Title: "Inspect mutation receipts", Description: "Read metadata-only MCP execution receipts by idempotency key or receipt id. Use this after a 502/timeout before retrying a mutating tool. Receipts store hashes and bounded status metadata, never raw arguments or stdout/stderr.", Annotations: readOnlyToolAnnotations(false), Handler: ctxToolHandler((*Runtime).requestReceipt)},
		{Name: "file_replace", Title: "Replace text in file", Description: toolfile.ToolDescription("Replace exact UTF-8 text in an existing file. This is the preferred ordinary text-edit tool for direct ChatGPT/Runtime MCP use; it cannot delete or move a file."), Annotations: mutatingToolAnnotations(false, false), Handler: ctxToolHandler((*Runtime).fileReplace)},
		{Name: "file_patch", Title: "Patch existing file", Description: toolfile.ToolDescription("Apply an update-only patch to an existing file. Structured patches may contain exactly one Update File operation and cannot add, delete, or move files; use the dedicated file tools for those operations."), Annotations: mutatingToolAnnotations(false, false), Handler: ctxToolHandler((*Runtime).filePatch)},
		{Name: "file_add", Title: "Add new file", Description: toolfile.ToolDescription("Create a new UTF-8 file without overwriting an existing path. Use file_replace/file_patch for existing files."), Annotations: mutatingToolAnnotations(false, false), Handler: ctxToolHandler((*Runtime).fileAdd)},
		{Name: "file_delete", Title: "Delete file or directory", Description: toolfile.ToolDescription("Delete a file, or a directory only when recursive=true."), Annotations: mutatingToolAnnotations(false, false), Handler: ctxToolHandler((*Runtime).fileDelete)},
		{Name: "file_move", Title: "Move file or directory", Description: toolfile.ToolDescription("Move a file or directory to a new path."), Annotations: mutatingToolAnnotations(false, false), Handler: ctxToolHandler((*Runtime).fileMove)},
		{Name: "file_edit", Title: "Edit file (compatibility)", Description: toolfile.EditDescription("Compatibility entrypoint combining replace, patch, add, delete, and move. Direct ChatGPT clients should prefer file_replace, file_patch, file_add, file_delete, or file_move so safety semantics are explicit."), Annotations: mutatingToolAnnotations(true, false), Handler: ctxToolHandler((*Runtime).fileEdit)},
		...
		{Name: "task_manage", Title: "Manage recoverable tasks", Description: "Persist substantial Runtime tasks and expose coarse-grained live step progress. Checkpoint sparingly at phase boundaries, real blockers, or meaningful recovery points; batch step updates instead of reporting every file, command, or tool call.", Annotations: mutatingToolAnnotations(false, false), Handler: ctxToolHandler((*Runtime).taskManage)},
		...
		{Name: "acp_start", Title: "Start ACP agent", Description: "Runtime ACP adapter: lazily select and start Codex, Claude, Grok, or an explicitly configured ACP agent, create a session, and optionally start its first prompt. Never call this unless the user explicitly requested ACP/another agent; ACP is dormant by default and is transport-only.", Annotations: mutatingToolAnnotations(false, true), Handler: func(ctx context.Context, r *Runtime, args map[string]any) (Result, error) {
			return r.acpAdapters.Start(ctx, args)
		}},
		{Name: "acp_resume", Title: "Resume ACP agent", Description: "Runtime ACP adapter: resume a persisted ACP session with its original agent and optionally start a new prompt. Use only after explicit user intent to delegate through ACP.", Annotations: mutatingToolAnnotations(false, true), Handler: func(ctx context.Context, r *Runtime, args map[string]any) (Result, error) {
			return r.acpAdapters.Resume(ctx, args)
		}},
		{Name: "acp_status", Title: "ACP agent status", Description: "Inspect Runtime ACP adapters, persisted sessions, current prompt events, and pending interactions without starting an agent process. ACP remains dormant unless acp_start/acp_resume is explicitly invoked.", Annotations: readOnlyToolAnnotations(false), Handler: func(ctx context.Context, r *Runtime, args map[string]any) (Result, error) {
			return r.acpAdapters.Status(ctx, args)
		}},
		{Name: "acp_stop", Title: "Stop ACP agent", Description: "Cancel the active prompt if present and close the selected Runtime ACP session. For custom adapters after a Runtime restart, pass the same explicit adapter command if automatic resolution is unavailable.", Annotations: mutatingToolAnnotations(true, true), Handler: func(ctx context.Context, r *Runtime, args map[string]any) (Result, error) {
			return r.acpAdapters.Stop(ctx, args)
		}},
		...
		{Name: "browser_session", Title: "Browser session", Description: "Start Runtime browser control through native CDP or attach an existing logged-in Chrome tab through Runtime Chrome Bridge. Supports tab lifecycle/status while preserving external Chrome when a Runtime session closes.", Annotations: mutatingToolAnnotations(true, true), Availability: requiresBrowser, Handler: ctxToolHandler((*Runtime).browserSession)},
		{Name: "browser_act", Title: "Browser actions", Description: "Run validated DOM/CDP browser actions: tab navigation, click, fill/type, select, upload, waits, network-aware actions and downloads. Returns live DOM/text plus screenshot and download Artifacts; an explicit Windows UIA semantic fallback may be supplied for operational browser failures.", Annotations: mutatingToolAnnotations(true, true), Availability: requiresBrowser, Handler: ctxToolHandler((*Runtime).browserAct)},
		{Name: "browser_step", Title: "Jev browser step", Description: "Use TypeSafe Jev over the current structured Chrome/DOM state to choose, risk-check, and optionally execute exactly one validated Runtime browser action. One request also evaluates completion, confirmation need, and page state; browser_act remains the actual CDP/Chrome Bridge executor. For best Jev accuracy, express goal in English when practical.", Annotations: mutatingToolAnnotations(true, true), Availability: requiresJevBrowser, Handler: ctxToolHandler((*Runtime).browserStep)},
		{Name: "browser_snapshot", Title: "Browser snapshot", Description: "Capture the active/requested CDP or Chrome-extension tab with URL, live DOM/text, tabs, focus, interactive elements, network/page diagnostics, and PNG screenshot Artifact.", Annotations: readOnlyToolAnnotations(true), Availability: requiresBrowser, Handler: ctxToolHandler((*Runtime).browserSnapshot)},
