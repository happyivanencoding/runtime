...
func requiresJev(cfg config.Config) bool     { return cfg.BrowserEnabled && cfg.TypeSafeAPIKey != "" }
...
func allToolSpecs() []ToolSpec {
	return bindToolSchemas(append([]ToolSpec{
		...
		{Name: "browser_step", Title: "Jev browser step", Description: "Use TypeSafe Jev over the current structured Chrome/DOM state to choose and execute exactly one validated Runtime browser action. Prefer this for fast semantic Chrome control when the next action is not already explicit; browser_act remains the actual CDP/Chrome Bridge executor. For best Jev accuracy, express goal in English when practical.", Annotations: mutatingToolAnnotations(true, true), Availability: requiresJev, Handler: ctxToolHandler((*Runtime).browserStep)},
